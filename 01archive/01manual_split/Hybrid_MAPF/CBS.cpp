...
CBSInstance::CBSInstance(Instance* in)
{
	// parameters
...
void CBSInstance::Solve(vector<int>& nodes, vector<int>& agents)
{
...
pair<int,int> CBSInstance::FindCoordinates(vector<vector<int> > graph, int node_id)
{
	for (size_t i = 0; i < graph.size(); i++)
	{
		for (size_t j = 0; j < graph[i].size(); j++)
